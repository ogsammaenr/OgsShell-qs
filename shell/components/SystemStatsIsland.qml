...
Rectangle {
  ...
  height: (typeof shellConfigService !== "undefined") ? Math.max(24, shellConfigService.barHeight - 4) : 30
  radius: height / 2
  ...
  // Smooth width transition based on the content row's size and island width scale
  width: (contentRow.width + 24) * ((typeof shellConfigService !== "undefined") ? shellConfigService.islandWidthScale : 1.0)
  Behavior on width {
